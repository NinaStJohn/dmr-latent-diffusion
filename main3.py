...
try:
    _ = pl.loggers.TestTubeLogger
except AttributeError:
    from pytorch_lightning.loggers import TensorBoardLogger as _TTLogger
    class _CompatTestTubeLogger(_TTLogger):
        """Drop-in alias so legacy code using TestTubeLogger keeps working."""
        pass
    pl.loggers.TestTubeLogger = _CompatTestTubeLogger
...
import numpy as np, torch.serialization as _ts
try:
...
class SetupCallback(Callback):
    ...
    def on_keyboard_interrupt(self, trainer, pl_module):
        if "trainer" in locals() and getattr(trainer, "global_rank", 0) == 0:
...
if __name__ == "__main__":
    ...
    try:
        ...
        # --- sane defaults for PL 1.x/2.x style keys ---
        # Map legacy mistakes like accelerator='ddp' to the right fields
        if trainer_config.get("accelerator", "") == "ddp":
            trainer_config["strategy"] = "ddp"
            trainer_config.pop("accelerator", None)

        # Merge any CLI-specified trainer args
        for k in nondefault_trainer_args(opt):
            trainer_config[k] = getattr(opt, k)

        # Decide device backend
        has_gpus = bool(trainer_config.get("gpus"))  # PL 1.x style; could be int/str/list
        if has_gpus:
            # Prefer CUDA w/ DDP when GPUs are requested
        ...
        "tensorboard": {
            ...
            "params": {"name": "tensorboard", "save_dir": logdir},
        },
        }
        ...
        default_callbacks_cfg = {
            ...
            "learning_rate_logger": {
                "target": "main3.LearningRateMonitor",
                "params": {
        ...
        # --- PyTorch 2.6 safe-unpickling allowlist: must run before any ckpt load ---
        from torch.serialization import add_safe_globals
        import numpy as np

        # PL callback classes that often appear inside ckpts
        from pytorch_lightning.callbacks.model_checkpoint import ModelCheckpoint
        from pytorch_lightning.callbacks.early_stopping import EarlyStopping
        from pytorch_lightning.callbacks.lr_monitor import LearningRateMonitor

        # Allowlist numpy dtypes + the PL classes
        to_allow = [
            np.dtype,
            getattr(np, "bool_", None),
            getattr(np, "int64", None),
            getattr(np, "float64", None),
            getattr(np.dtypes, "Float64DType", None),  # explicitly named in your error
            ModelCheckpoint,
            EarlyStopping,
            LearningRateMonitor,
        ]
        add_safe_globals([t for t in to_allow if t is not None])
        # ----------------------------------------------------------------------------- 
        ...
        if not cpu:
            gpus_cfg = getattr(lightning_config.trainer, "gpus", None)
            if isinstance(gpus_cfg, str):
                ngpu = len([s for s in gpus_cfg.strip().split(",") if s])
            elif isinstance(gpus_cfg, (list, tuple)):
                ngpu = len(gpus_cfg)
            elif isinstance(gpus_cfg, int):
                ngpu = gpus_cfg
            elif gpus_cfg is None:
                ngpu = 0
            else:
                import torch
        ...
        # run
        resume_path = getattr(opt, "resume_from_checkpoint", None) if opt.resume else None
        if opt.train:
            try:
                ...
                melk()
                raise
        test_ckpt = None if opt.train else resume_path
        if not opt.no_test and not trainer.interrupted:
            trainer.test(model, data, ckpt_path=resume_path)
    except Exception:
