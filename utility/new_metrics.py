...
def plot_pdf_cdf_bar(data, metric_name, metrics_folder, train_folder, sigma=2, fixed_axis_scaling=False):
    ...
    def compute_mean_and_range(dataset):
        ...
        cdfs = np.array([np.interp(grid, d.bin_centers, d.cdf, left=0, right = 1) for d in dataset])
...
def main(train_folder, test_folder, org_image, rec_image, metrics_folder, fid_only, fixed_axis_scaling=False):
    ...
    if train_folder:
        real_images = load_images(train_folder)
        synth_images = load_images(test_folder)
    ...
    if not fid_only:
        ...
        if train_folder:
            ...
            for i, im in enumerate(real_images):
                ...
                metrics["two_point_correlation"]['train'].append(ps.metrics.two_point_correlation(np_array))
            ...
            print("Beginning metric calculations for synthetic images")
            for i, im in enumerate(synth_images):
                ...
                metrics["two_point_correlation"]['test'].append(ps.metrics.two_point_correlation(np_array))
